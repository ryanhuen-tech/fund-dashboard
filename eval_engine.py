# eval_engine.py - 100% 依據用戶原圖風控標準之真實數據動態精算引擎
import re
...
def eval_fund_dimensions(fund_obj, fund_type):
    ...
    if "本金補貼" in p2_delta or "補貼" in short_tag or code in ["Z15", "ZP4", "Z05"]:
        s1 = 12.5
        d1 = f"🟡 純收益覆蓋率介於 60%–100% (息差: {p2_str}，存在部分本金補貼派息)"
    ...
    if p4_val is None:
        s5 = 5.0
        d5 = f"🟡 平均修正/有效存續期: {p4_str} (位於 3.5–7 年區間)"
        b5 = "<span class='badge-yellow'>🟡 存續期 3.5-7年</span>"
    elif p4_val <= 3.5:
        s5 = 10.0
        d5 = f"🟢 平均修正/有效存續期: {p4_val:.2f} 年 (≤ 3.5年)，高抗升息力"
    ...
    # ==========================================================================
    # 九、衍生工具結構風險 (10分)
    # +10分: 無高風險結構性衍生品 (淨曝險 ≤ 50%)
    # +0分: 剛性否決: 144A ELN / TRS 雜項本金 ≥ 20%
    # ==========================================================================
    ...
    # ==========================================================================
    # 十、不對稱策略風險 (15分)
    # +15分: 零生息採用 Short Options (無不對稱風險)
    # +7.5分: 少量對沖期權 (本金 < 10%)
    # +0分: 剛性否決: 大幅賣出期權 (本金 ≥ 10%)
    # ==========================================================================
