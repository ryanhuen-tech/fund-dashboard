# eval_engine.py - 100% 嚴格真實數據驅動引擎（零虛擬/零預設補填）
import re

def parse_float_from_str(val_str):
    """從字串中精確提取浮點數，若無真實數字則返回 None"""
...
def eval_concentration_risk(fund_obj):
    """第 3 項：集中度風險 (上限 5 分) - 僅依據 p6 真實數據評估"""
    kpis = fund_obj.get("kpis", {})
    p6_val = parse_float_from_str(kpis.get("p6"))
    
    if p6_val is None:
        return 0.0, "⚠️ 官方 Factsheet 正本未披露前十大持倉比率 (0 分)"
...
def eval_asymmetric_risk(fund_obj, fund_type):
    ...
    if final_score == 15.0:
        ...
        return final_score, f"⚠️ 存在結構次級風險: {', '.join(reasons)} (扣 {int(deductions)} 分)", "<span class='badge-yellow'>⚠️ 黃色警示</span>"
    else:
        return final_score, f"🚨 具備嚴重下行不對稱風險: {', '.join(reasons)} (扣 {int(deductions)} 分)", "<span class='badge-red'>🚨 紅色危險</span>"

def process_fund_risk_scores(funds_dict):
    """為所有基金徹底按真實數據計算風控得分，不為缺失維度偽造分數"""
    for code, fund in funds_dict.items():
        cat_type = fund.get("category", "債券基金")
        
        # 取得模組原有真實 radar_scores，無則初始為全 0（非假滿分）
...
def generate_dynamic_eval_table(fund_obj, fund_type):
    """生成單一基金深度剖析明細表（100% 客觀真實）"""
    scores = fund_obj.get("radar_scores", [0.0]*10)
    ...
    c_badge = "<span class='badge-green'>🟢 綠色健康</span>" if c_score == 5.0 else "<span class='badge-yellow'>⚠️ 黃色警示</span>" if c_score > 0 else "<span class='badge-red'>🚨 數據未披露/高風險</span>"
    l_badge = "<span class='badge-green'>🟢 綠色健康</span>" if l_score == 5.0 else "<span class='badge-yellow'>⚠️ 黃色警示</span>" if l_score > 0 else "<span class='badge-red'>🚨 數據未披露/高風險</span>"
