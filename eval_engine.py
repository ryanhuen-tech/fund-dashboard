# eval_engine.py - 徹底修復非債券基金 0 分 Bug 之全資產風控引擎

def generate_dynamic_eval_table(curr_fund, category_type):
    """根據資產類別 (債券 / 股票 / 股債混合) 動態生成高鑑別度評估明細表，絕不回傳空陣列"""
    ...
    if is_bond or (code.startswith("Z") and code not in ["Z01", "Z03", "Z07", "Z33"]):
        ...
        if code in ["Z12", "Z15"]:
            score_4 = 2.5 if code == "Z15" else 5.0
            badge_4 = "<span class='quality-badge-green'>✔ 無融券槓桿 (<=105%)</span>" if score_4 == 5.0 else "<span class='quality-badge-yellow'>🟡 比率 105%-120%</span>"
    ...
    # =========================================================================
    # 🎯 2. 股票型基金評估準則 (Equity Funds Standard - 例如 Z01)
    # =========================================================================
    elif is_equity or code == "Z01":
        score_e1 = 25.0
        score_e2 = 15.0  # Z01 為高息股票，護城河優良
        score_e3 = 5.0
        score_e4 = 5.0
        score_e5 = 7.5
        score_e6 = 5.0
    ...
    # =========================================================================
    # 🎯 3. 股債混合型基金評估準則 (Balanced Funds Standard - 例如 Z03, Z07, Z33)
    # =========================================================================
    else:
        score_m1 = 22.5
        score_m2 = 12.5
        ...
        score_m8 = 2.5
        score_m9 = 10.0
        score_m10 = 10.0 if code in ["Z03", "Z07"] else 15.0  # Z03/Z07 採 Covered Call 扣 5分
