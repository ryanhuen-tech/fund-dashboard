...
# 5. 上方 7 大核心數據名片 (名片卡片列)
if fund_type == "債券基金":
    st.markdown(f"""
    <div class="kpi-grid-7">
        <div class="kpi-card-custom">
            <div class="kpi-card-title">現時派息率</div>
            <div class="kpi-card-value">{curr_fund['kpis']['p1']}</div>
            <div class="kpi-card-sub">年化分派</div>
        </div>
        <div class="kpi-card-custom">
            <div class="kpi-card-title">息差 (收益率差)</div>
            <div class="kpi-card-value">{curr_fund['kpis']['p2']}</div>
            <div class="kpi-card-sub">派息覆蓋佳</div>
        </div>
        <div class="kpi-card-custom">
            <div class="kpi-card-title">平均持有債務評級</div>
            <div class="kpi-card-value">{curr_fund['kpis']['p3']}</div>
            <div class="kpi-card-sub">高收益債</div>
        </div>
        <div class="kpi-card-custom">
            <div class="kpi-card-title">續存率 / 有效期</div>
            <div class="kpi-card-value">{curr_fund['kpis']['p4']}</div>
            <div class="kpi-card-sub">存續期 (久期)</div>
        </div>
        <div class="kpi-card-custom">
            <div class="kpi-card-title">手持現金比率</div>
            <div class="kpi-card-value">{curr_fund['kpis']['p5']}</div>
            <div class="kpi-card-sub">流動性充沛</div>
        </div>
        <div class="kpi-card-custom">
            <div class="kpi-card-title">前十大發行人佔比</div>
            <div class="kpi-card-value">{curr_fund['kpis']['p6']}</div>
            <div class="kpi-card-sub">極度分散</div>
        </div>
        <div class="kpi-card-custom">
            <div class="kpi-card-title">槓桿比率</div>
            <div class="kpi-card-value">{curr_fund['kpis']['p7']}</div>
            <div class="kpi-card-sub">無顯著借貸</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
else:
...
# 6. 中間核心區：【左圖】深色互動動態雷達圖 + 十大持倉 vs 【右圖】1:1 HTML 表格
col_left, col_right = st.columns([1, 1.4], gap="medium")

# 👈 【左欄】：恢復使用 Plotly 動態深色互動式雷達圖
with col_left:
    ...
    with tab1:
        if fund_type != "債券基金":
            ...
            # 💡 Plotly 深色主題動態互動雷達圖 (包含動態 Hover 與移動縮放)
            ...
            fig_radar.update_layout(
                ...
                polar=dict(
                    radialaxis=dict(visible=True, range=[0, 20], showticklabels=False),
...
with col_right:
    ...
    if fund_type == "債券基金":
        ...
        <div class="summary-footer">
            ...
            <span class="status-badge-green" style="font-size: 12px; padding: 4px 10px;">82.5% (健康)</span>
        </div>
        """
        st.markdown(html_table, unsafe_allow_html=True)
    else:
        # 📈 股票基金 / 股債混合基金 專屬「底層資產」深度風險評估表
        html_table = f"""
        <table class="custom-table">
            <thead>
                <tr>
