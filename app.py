    ...
    if len(df_matrix) == 0:
        ...
        with filter_col2:
            sort_by_col = st.selectbox(
                "🔀 請選擇表格排序依據 (Sort By)：",
                ["風險與回報對比指數", "近1年總回報 (%)", "近3年年化總回報 (%)", "上月年化派息率 (%)", "綜合風險總分", "代號", "晨星評級"],
                index=0
            )
        with filter_col3:
        ...
        <div style="overflow-x: auto;">
            <table class="custom-table" style="min-width: 1650px;">
                <thead>
                    <tr>
                        <th>代號</th><th>基金名稱</th><th>基金類別</th><th>上月派息率 (%)</th><th>近1年總回報 (%)</th><th>近3年年化回報 (%)</th><th>晨星評級</th><th>綜合風險總分</th><th>風險與回報對比指數 🏆</th><th>一、派息可持續性 (20)</th><th>二、底層質素 (15)</th><th>三、集中度 (5)</th><th>四、信用/風控 (10)</th><th>五、槓桿/天花板 (10)</th><th>六、敏感度/Beta (10)</th><th>七、流動性 (10)</th><th>八、匯率風險 (10)</th><th>九、區域風險 (5)</th><th>十、淨值波動 (5)</th>
                    </tr>
                </thead>
        ...
        with col_compare_right:
            ...
            fig_rank = px.bar(
                ...
                title="風險與回報對比指數 (更高＝風險效率最高)", color='風險與回報對比指數', color_continuous_scale='Greens', template="plotly_white"
            )
            fig_rank.update_layout(height=450, yaxis_title="基金代號", xaxis_title="指數值")
...
with top_tab2:
    ...
    with st.expander(f"🏢 點擊展開 / 折疊：基金公司背景簡介 — {curr_fund['company_name']}", expanded=False):
        st.markdown(f'<ul class="company-profile-list">{company_profile_html}</ul>', unsafe_allow_html=True)

    st.markdown("""
        <div class="data-disclaimer-note">
            <b>📑 數據來源聲明備註：</b> 本 Dashboard 內所有財務數據、持倉比率、派息成分與營運損益，均完全依據<b>基金官方發布之基金月報 (Factsheet)、派息分派紀錄 (Dividend Distribution History) 及年度財務報告 (Annual Report)</b> 客觀建檔分析。
        </div>
    """, unsafe_allow_html=True)
    ...
    if show_g2:
        ...
        with g2_c1: st.metric(label="平均持有評級/屬性", value=curr_fund['kpis']['p3'], delta=p3_delta, delta_color=p3_color)
        with g2_c2: st.metric(label="存續期/Beta敏感度", value=curr_fund['kpis']['p4'], delta="風險敏感指標", delta_color="normal")
        with g2_c3: st.metric(label="手持現金/衍生品比率", value=curr_fund['kpis']['p5'], delta=p5_delta, delta_color=p5_color)
        with g2_c4: st.metric(label="總持有資產數量", value=curr_fund['holdings_count'], delta="底層持倉分散度", delta_color="normal")
        with g2_c5: st.metric(label="前十大發行人/持股佔比", value=curr_fund['kpis']['p6'], delta="集中度管控", delta_color="normal")
        with g2_c6: st.metric(label="槓桿比率", value=curr_fund['kpis']['p7'], delta="借貸/衍生品膨脹率", delta_color="normal")
