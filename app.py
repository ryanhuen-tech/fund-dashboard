        ...
        if not gemini_api_key:
            ...
            try:
                with st.spinner("🤖 Google Gemini AI 正在連線並智能解析 PDF..."):
                    ...
                    data_source = f"Google Gemini AI [{selected_model}] 解析：{uploaded_file.name}"
                    st.success(f"🎉 AI 解析成功！(模型: {selected_model}) | 基金：{fund_name_zh} | ISIN: {fund_isin} | 久期: {duration_val}年 | 現金: {cash_val}% | 來自資本: {roc_val}%")
